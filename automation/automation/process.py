...
async def process_hardcoded(directory: Path, language: LanguageInfo, config: Config):
...
async def process_objects(directory: Path, language: LanguageInfo, config: Config):
    ...
    with errors_file_path.open("w", encoding="utf-8") as errors_file:
        csv_objects_data = await convert_objects(po_data, errors_file)

    if errors_file_path.stat().st_size == 0:
        errors_file_path.unlink()

    return csv_objects_data
...
async def process(language: LanguageInfo, config: Config):
    ...
    csv_data = await process_hardcoded(csv_directory, language, config)
    ...
    csv_objects_data = await process_objects(csv_with_objects_directory, language, config)

    file_path = csv_with_objects_directory / "dfint_dictionary.csv"
    async with aiofiles.open(file_path, "wb") as csv_file:
        await csv_file.write(codecs.encode(csv_data, encoding=language.encoding))
        await csv_file.write(codecs.encode(csv_objects_data, encoding=language.encoding))
